...
class JsonValidator
{
    ...
    {
        $entityName = ($entityName==null) ? 'root' : $entityName;
            ...
            if (is_array($schema->items)) {
                foreach($entity as $index => $node) {
                    ...
                    if (!$nodeValid) {
                        ...
                        $anonymousFunc = create_function('$item', 'return $item->type == "object" ? "object (schema)" : $item->type;');
                        
                        $allowedTypes = array_map($anonymousFunc, $schema->items);
    ...
    protected function checkDisallow($entity, $schema, $entityName)
    {
        if (isset($schema->disallow) && $schema->disallow) {
            ...
            if (!$valid) {
                ...
                $anonymousFunc = create_function('$item', 'return is_object($item) ? "object (schema)" : $item;');
                
                $disallowedTypes = array_map($anonymousFunc, is_array($schema->disallow) ? $schema->disallow : array($schema->disallow));
